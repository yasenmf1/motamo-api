...
<span class="grp"><button class="prod" onclick="doProduce()">① Производство</button><button class="acc" onclick="doAccounts()">② Сметки</button><button class="alt" onclick="loadAccounts()" title="Изтегля реалните сметки за ДНЕС (разнос ден) с текущите количества — за ③ Стокова">↻ Изтегли сметки (днес)</button><button class="acc" onclick="doStokova()">③ Стокова</button></span></header>
...
// Тегли автоматично за ДНЕС (деня на разноса) — не иска дата от потребителя.
function loadAccounts(){msg('Изтеглям реалните сметки за днес…');api({action:'load_accounts'}).then(function(j){if(!j.ok){msg('Грешка: '+(j.error||''),'err');return}shops=j.shops||[];renderGrid();$('planbox').innerHTML='';if(!shops.length){msg('Няма сметки за днес ('+j.date+'). Първо направи ② Сметки (в навечерието).','err');return}msg('Изтеглени '+shops.length+' реални сметки за днес ('+j.date+') — с номера + текущи количества. Провери и натисни ③ Стокова.','ok')}).catch(function(e){msg('Мрежова грешка: '+e,'err')})}
