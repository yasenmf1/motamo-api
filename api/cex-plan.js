...
function doStokova(){var sel=selShops().filter(function(s){return s.account_id});if(!sel.length){msg('Няма сметки за стокова. Тикни обектите и първо ② Сметки.','err');return}var pd=$('pdate').value||$('date').value;if(!confirm('Ще СЪЗДАМ стокови разписки за '+sel.length+' обекта (тикнатите), с дата '+pd+'.\\nПродължавам?'))return;msg('Правя стоковите за '+sel.length+' обекта…');var done=0,okc=0,errs=[];sel.forEach(function(s){var id=s.account_id;api({action:'create_stokova',account_id:id,date:pd}).then(function(j){done++;if(j.ok)okc++;else errs.push('#'+id+': '+(j.error||''));if(done===sel.length){msg('✓ Стокови: '+okc+' готови'+(errs.length?(' · грешки: '+errs.join(' | ')):''),errs.length?'err':'ok')}}).catch(function(e){done++;errs.push('#'+id+': мрежа');if(done===sel.length)msg('Грешки: '+errs.join(' | '),'err')})})}
