...
<span class="grp"><button class="alt" onclick="calc()">Изчисли</button><button class="alt" onclick="openPicker()" title="Добави магазин, който днешният разнос не е заредил">+ Магазин</button><button class="alt" onclick="clearGrid()" title="Изтрий попълненото и започни начисто">🗑 Изчисти</button></span>
...
function renderCreated(cr){var h='<h2>Създадени сметки</h2><table><tr><th class="shop">Магазин</th><th>сметка №</th><th>артикули</th><th>статус</th></tr>';cr.forEach(function(c){var full=(c.client||'')+(c.rep?(' · '+c.rep):'');var st=c.ok?'✓':('<span style="color:#c00" title="'+esc(c.error||'')+'">'+esc(c.error||c.skipped||'грешка')+'</span>');h+='<tr><td class="shop" title="'+esc(full)+'">'+esc(shortName(c.client,c.rep))+'</td><td>'+(c.account_id||'—')+'</td><td>'+(c.items||0)+'</td><td>'+st+'</td></tr>'});$('planbox').innerHTML=h+'</table>'}
